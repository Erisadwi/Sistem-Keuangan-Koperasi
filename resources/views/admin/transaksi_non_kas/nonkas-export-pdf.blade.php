...
<head>
    <meta charset="utf-8">
    <title>Laporan Transaksi Non Kas</title>
    <style>
        body {
...
<table>
    ...
    @forelse($data as $row)
        @php
            $akunDebit = $row->details->firstWhere('debit', '>', 0)?->akun;
            $akunKreditList = $row->details->where('kredit', '>', 0);
        ...
        <tr>
            ...
            <td>{{ $akunDebit->nama_AkunTransaksi ?? '-' }}</td>

            <td>
                <ul>
                    @foreach($akunKreditList as $k)
                        <li>{{ $k->akun->nama_AkunTransaksi ?? '-' }}</li>
                    @endforeach
                </ul>
            </td>
    ...
    @empty
        <tr>
            <td colspan="8"><em>Tidak ada data transaksi</em></td>
        </tr>
    @endforelse
