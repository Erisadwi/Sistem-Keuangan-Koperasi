...
$label = $start && $end ? $fmt($start) . ' - ' . $fmt($end) : 'Tanggal';
@endphp

<form method="{{ strtoupper($method)=='POST' ? 'POST' : 'GET' }}"
      action="{{ $action }}" id="{{ $uid }}_form" class="df-inline df-compact">
...
(function(){
  ...
  trig.addEventListener('click', ()=> pop.hidden ? pop.hidden=false : pop.hidden=true);
  ...
  btnCancel?.addEventListener('click', ()=> pop.hidden=true);
})();
</script>
